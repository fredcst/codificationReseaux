  ...
  switch(error_type)
    {
      ...
      case 5: includeError5(string, size); break;
      default:
        {
...
void includeError5(char *string, int size)
{
  ...
  int rate = ERROR_RATE * 100;
  for(i=0; i<size; i++)
    {
      if((rand() % 100) < rate){
      	char error = 0;
      	char exponent = rand() % 8;
      	error = pow(2, exponent);
      	string[i] = string[i] ^ error ;	
      }   
