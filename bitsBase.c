...
void montreFILEBYTE(FILE *fp,BYTE b)
{
	...
	for(i=0;i<8;i++)
	{
		r=r0>>i;	// positionne 1 en bit 7-i;
		d=b&r;//sélectionne bit r de b pour tester si est null or non
		if(fprintf(fp,"%d",(d==0)?0:1)<0){perror("montreFILEBYTE");exit(-1);};//si est null écrit 0, sinon 1
	}
	
	
	if(fprintf(fp,"\n")<0){perror("montreFILEBYTE");exit(-1);};
...
void montreFILEBYTESansNL(FILE *fp,BYTE b)
{
	...
	for(i=0;i<8;i++)
	{
		r=r0>>i;	// positionne 1 en bit 7-i;
		d=b&r;//sélectionne bit r de b pour tester si est null or non
		if(fprintf(fp,"%d",(d==0)?0:1)<0){perror("montreFILEBYTESansNL");exit(-1);};//si est null écrit 0, sinon 1
	}
...
void montreFILEHNible(FILE *fp,BYTE b)
{
	...
	for(i=0;i<4;i++)
	{
		r=r0>>i;	// positionne 1 en bit 7-i;
		d=b&r;//sélectionne bit r de b pour tester si est null or non
		if(fprintf(fp,"%d",(d==0)?0:1)<0){perror("montreFILEHNible");exit(-1);};//si est null écrit 0, sinon 1
	}
	if(fprintf(fp,"\n")<0){perror("montreFILEHNible");exit(-1);};
return;
}
/**
...
void montreFILEHNibleSansNL(FILE *fp,BYTE b)
{
	...
	r=r0>>i;	// positionne 1 en bit 7-i;
	d=b&r;//sélectionne bit r de b pour tester si est null or non
	if(fprintf(fp,"%d",(d==0)?0:1)<0){perror("montreFILEHNibleSansNL");//si est null écrit 0, sinon 1
			exit(-1);}
	}
...
*/
	...
	unox=uno;//se positionne a droite en pos 0
	for(i=0;i<n;i++)
	{//couvre tout le codeword de n bits
		unox =uno <<i ; // positionne 1 en position i
...
{
	printf("%s\t",frase);montreUINT(len,ux);
	return;
}
